...
public class Salarie {
	...
	public void setM_nCategorie(int m_nCategorie) throws CategorieSalarieException {
		if (!(m_nCategorie < 0) && !(m_nCategorie > 3)) {
			throw new CategorieSalarieException("Les catégories sont 1, 2 et 3!");
		} else {
			this.m_nCategorie = m_nCategorie;
		}
	}
	...
	public void setM_dSalaire(double m_dSalaire) throws SalarieSalaireException {
		if (m_dSalaire < 0) {
		throw new SalarieSalaireException("Le salaire doit etre supérieur à zero!");
		} else {
			this.m_dSalaire = m_dSalaire;
		}
	}
